...
export const api = {
  ...
  // 5. 保存 (Save)
  saveProcedure: async (procedure_id: number, new_content: string, change_summary: string): Promise<SaveResponse> => {
    await delay(600);
...
[IBM][CLI Driver][DB2/LINUXX8664] SQL0911N  The current transaction has been rolled back because of a deadlock or timeout.  Reason code "68".  SQLSTATE=40001`
  ...
  // 6. 新建 (Create)
  createProcedure: async (data: { name: string, db: string, owner: string, content: string }): Promise<CreateResponse> => {
    await delay(800);
    // Mock success
    return {
      flag: true,
